...
##### Session-specific delivery (optional sessionKey)

By default, main jobs deliver to the agent's main session (resolved via `agentId`).
You can override this with an explicit `sessionKey` to target a specific conversation:

```json
{
  "name": "Reminder with context",
  "schedule": { "kind": "at", "at": "2026-02-21T15:30:00Z" },
  "sessionTarget": "main",
  "sessionKey": "agent:main:telegram:group:-1003707749614:topic:1053",
  "wakeMode": "now",
  "payload": { "kind": "systemEvent", "text": "Remember: check the logs" }
}
```

When `sessionKey` is present, the job ignores `agentId` resolution and sends the
payload directly to that session. This allows:

- **Context preservation**: reminders execute in the same conversation where they were created.
- **Multi-chat targeting**: a single agent can deliver cron jobs to different channels/topics.
- **Isolated job summaries**: when an isolated job has `sessionKey`, its summary is posted
  back to that specific session instead of the default main session.

Use cases:

- _"Remind me in 30 minutes to check this"_ → set `sessionKey` to the current conversation.
- Topic-specific reminders in Telegram forums or Discord channels.
- Per-channel isolated jobs (e.g., daily summary for a specific Slack channel).

Backward compatibility: jobs without `sessionKey` work as before, resolving the target
session via `agentId`.
