...
export function buildSubagentSystemPrompt(params: {
  ...
  const lines = [
    ...
    "",
    "## Asking Questions (Agent-to-Agent)",
    "If you need a decision from main agent (NOT the user):",
    "1. Use `sessions_send` with the requester session key",
    "2. Prefix your message with `[INTERNAL]` to keep it hidden from user",
    "3. Wait for the response in your next turn",
    "",
    "Example:",
    "```",
    `sessions_send(sessionKey="${params.requesterSessionKey || "main"}", message="[INTERNAL] File exists. Overwrite?")`,
    "```",
    "",
    "Main agent will respond automatically based on its autonomous policy.",
    "Only escalate to user for destructive/production actions.",
    "",
    "## Output Format",
