...
export function loadSessionStore(
  ...
  // On Windows, writeFile is non-atomic. A concurrent readFileSync can observe
  // a partially-written file, causing JSON.parse to throw. Retry once after a
  // brief delay to allow the in-flight write to complete.
  const maxReadAttempts = process.platform === "win32" ? 2 : 1;
  for (let attempt = 0; attempt < maxReadAttempts; attempt++) {
    try {
      ...
      if (attempt + 1 < maxReadAttempts) {
        // Brief synchronous delay (~100ms) before retrying.
